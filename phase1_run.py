...
def train_and_evaluate():
    ...
    train_loader = DataLoader(train_data, batch_size=128, shuffle=True) #prev batch = 64
    val_loader = DataLoader(val_data, batch_size=256, shuffle=False) #prev batch = 64
    ...
    total_steps = len(train_loader) * kl_anneal_epochs
    global_step = 0
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in train_loader:
            ...
            _, alpha = model(input_ids=input_ids, attention_mask=attn_mask)
            global_step += 1
            anneal = min(1.0, global_step / total_steps)
